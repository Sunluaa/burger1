...
export const selectOrderDataByNumber = (number: string) => (state: RootState) => {
  if (state.order.order?.length) {
    const data = state.order.order.find((order) => order.number === Number(number));
    if (data) return data;
  }
  if (state.feed.items?.orders?.length) {
    const data = state.feed.items.orders.find((order) => order.number === Number(number));
    if (data) return data;
  }
